...
export const scenes: Scene[] = [
  ...
  },
  {
    ...
    motionPrompt:
      "0–2s: the shot opens exactly on the still frame it starts from and barely moves — the cyan transaction atom already large, resolved and holding at the centre of calm dark space with its labels already legible, only a thin band of warp streaks left smearing at the extreme outer edges, the parent block's gold orb clusters blurred at the corners; the warp is over and behind us, and those last edge streaks simply slow and fade. 2–8s: the edge streaks thin away entirely; the atom holds centre as its detail sharpens; it stays facing the camera the whole time and only sways gently — no more than about thirty degrees left, then back and about thirty degrees right — a slow parallax rock that shows the concentric shells are three-dimensional without ever turning edge-on or flattening; the shells stay open and round and the labels (nucleus version/locktime, left band inputs, right band outputs) stay flat, upright and readable throughout. 8–12s: the sway settles back to centre and the atom comes to rest, front-on and legible. The atom never spins a full turn and never presents its edge; keep it three-quarter to front-facing at all times.",
  ...
  },
  {
    ...
    motionPrompt:
      "0–2s: opens exactly on the still frame it starts from — UTXO A (Alice, 5.0 BTC) as the large bright sphere at left, the labeled transaction atom at right with its single input bead, the one thin white-cyan filament already spanning the gap between them, and the two output streams (Bob 3.5, Change 1.5) just beginning to bud from the atom; every label already legible. 2–8s: a red SPENT seal closes over UTXO A; the two output streams finish crystallizing into their nucleation points — UTXO B (Bob) on the right, the smaller Change UTXO curving back toward Alice on the lower left; their sizes hold the 5.0 = 3.5 + 1.5 proportion. 8–11s: nothing travels along the filament — UTXO A dims to a low ember exactly as the two new outputs brighten, both ends responding together. 11–13s: the composition holds and settles, everything at rest, labels sharp, a calm final beat. Very slow dolly push-in throughout; the atom stays front-facing and never spins.",
    camera: ["Dolly In"],
    models: { primary: "Seedance 2.0", alt: "Seedance 2.0 Mini" },
    narration:
  ...
  },
  {
    ...
    motionPrompt:
      "0–5s: transaction atoms light up as nodes on a violet web — a filament points from TX0 output[1] to TX1 input[0], which branches into output[0] and output[1]; nothing travels along the filament — both ends respond together. 5–10s: the web branches outward into deep space like a cosmic filament structure. 10–13s: at the bright contested node two filaments point at the same already-spent output; the white one lands and wins, the red one collapses just short. 13–15s: the graph holds still, the surviving link glowing, a calm final beat.",
