...
export const scenes: Scene[] = [
  ...
  },
  {
    ...
    durationSeconds: 15, // narration ~13s
    keyframePrompt:
      "Astrophotography, photographed through a telescope on a long exposure: a mostly black deep-space field, with a faint dusty magenta-violet nebula drifting far behind everything, heavily out of focus and subtle — not vivid, not saturated; sparse stars over large areas of empty black. Several completed Bitcoin-gold block-bodies (warm orange-gold, hex #F7931A) hang scattered at clearly different heights and depths — one low and near, the next high and far back, the next lower again — a staggered zigzag strewn across the frame, each separated from its neighbours by a wide gap of empty black; each is an irregular rounded clump of small glowing transaction orbs bonded together, their surfaces turbulent and granulated like a real star's convecting photosphere, with a glowing crystalline block-header core at its heart; the nearest clump is largest and brightest and reads immediately as the subject. Each gap is crossed by its own short, separate, thin white-cyan filament, each running at its own diagonal angle from one clump's header core to the outer surface of the clump before it, so that the filaments read as a broken zigzag of individual segments strung between the clumps; nothing travels along the filament — both ends respond together; ₿ symbols on the clumps. Dark, restrained, subtle color, deep blacks, film grain, volumetric glow, cinematic, ultra detailed; no text labels, no planet surface, no landscape, no people; the ₿ Bitcoin symbol only — no Ethereum, no other cryptocurrency logos, no diamond or octahedron gem, no flat honeycomb hexagon lattice",
    motionPrompt:
