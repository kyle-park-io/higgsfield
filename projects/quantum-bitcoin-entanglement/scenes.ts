...
export const scenes: Scene[] = [
  ...
  },
  {
    ...
    keyframePrompt:
      'Scientific visualization, a mostly dark and restrained frame: only in a thin band along the extreme outer edges do radial hyperspace light streaks in cyan, violet and white still smear inward, motion-blurred, dying out well before they reach the middle — we have just crash-zoomed through the gold block surface and the tunnel is already behind us. The centre of the frame is calm dark space with deep blacks, and floating there is a single richly detailed glowing cyan transaction atom drawn as a precise ornate atomic diagram: a bright textured cyan nucleus labeled "version / locktime", surrounded by multiple concentric orbital shells with many orbiting electrons, glowing energy filaments and a luminous particle field; a left orbital band labeled "inputs (prev txid, vout, sequence, scriptSig/witness)" and a right orbital band labeled "outputs (amount, scriptPubKey)"; framing the atom at the frame edges, faint blurred fragments of the parent block-molecule we have just come through — clusters of many small warm orange-gold (#F7931A) plasma orbs bonded together, heavily out of focus and dim; behind everything a muted violet-to-teal nebula, low density, heavily defocused; dense starfield, dark near-field, volumetric glow, cinematic, ultra detailed, legible small text labels; no physical room or lab, no planet surface, no landscape, no people, no coins, no flat gold discs, no medallions, no ₿ symbols at the edges',
    motionPrompt:
  ...
  },
  {
    ...
    keyframePrompt:
      "Scientific visualization: several glowing cyan transaction atoms arranged as a directed web — faint violet dark-matter-like filaments studded with the atoms as glowing nodes, branching outward into deep space (not a straight line); each filament points from one transaction's output node to the next transaction's input node — one thin, intensely bright white-cyan line whose direction is marked by a small arrowhead at the input end; nothing travels along the filament — both ends respond together; the web is dense and richly branching, many transaction atoms deep, and it reads left to right in time: \"TX0\" sits on the left with \"output[0]\" and \"output[1]\" branching away from it, and one of those outputs runs on rightward into \"input[0]\" of \"TX1\", which branches again into its own outputs — this whole spreading structure is the texture of the frame. Against that texture one single node stands out as the unmistakable focal point, drawn much larger and brighter than every other node with the rest of the web falling away darker around it: the contested output. Two filaments reach that one node from two different transactions on opposite sides at once, a race for the same already-spent output — one is intact white-cyan and its arrowhead lands cleanly on the node, the winner; the other has gone red along its whole length, the only red anywhere in the frame, its arrowhead never forming and its line fading out into cool embers just short of the node it failed to reach, the loser, left unattached to the graph; behind everything a muted violet-to-teal nebula, low density, heavily defocused; dense starfield, dark near-field, volumetric glow, cinematic, ultra detailed, legible small text labels; no physical room or lab, no planet surface, no landscape, no people",
    motionPrompt:
  ...
  },
  {
    ...
    durationSeconds: 13, // narration ~11s
    keyframePrompt:
      'Scientific visualization, extreme macro: a tall vertical transparent stack-tube shaped from glowing violet energy (NOT lab glassware) floating in deep space — no laboratory, no benches, no monitors, no furniture; resting at the bottom of the tube, exactly two violet-magenta subatomic quark-like particles, well apart from each other with clear dark space between them — the lower one labeled "Sig", the one above it labeled "PubKey", each label small, sharp and set clear of everything around it; the five script steps run up the tube in this order from bottom to top, each labeled with exactly one of these spellings — these exact five strings and no other opcode names — and each drawn as a visibly different operation so the shape itself shows what the step does: "OP_DUP", where one particle splits into two identical twins side by side; "OP_HASH160", where one of those twins passes through a bright compression ring and comes out transformed into a dense angular fragment of a different shape; "<PubKeyHash>", a second identical angular fragment dropping in from the side to meet it; "OP_EQUALVERIFY", where those two angular fragments slot together like matching halves and flash green at the seam; "OP_CHECKSIG", where the remaining particles merge into a single bright sphere; a single green "TRUE" particle glowing at the top; behind everything a sparse deep teal and blue nebula so the violet stack reads clearly; dense starfield, dark near-field, volumetric glow, cinematic, ultra detailed, legible small text labels; no planet surface, no landscape, no people',
    motionPrompt:
