...
export const scenes: Scene[] = [
  ...
  },
  {
    ...
    keyframePrompt:
      'Scientific visualization, ornate atomic diagram rendered entirely as glowing cyan particles and atoms (no machinery, no robots): on the left a cyan output atom labeled "UTXO A / Alice (5.0 BTC)", already dimmed to a low ember glow and stamped with a red "SPENT" seal; on the right a new transaction atom, already burning at full bright cyan; the two are separated across dark space and joined by one thin, intensely bright white-cyan energy filament — the input\'s reference to the previous output; nothing travels along the filament, no coin, no particle — both ends respond together; a newly forming cyan output atom "UTXO B / Bob (3.5 BTC)" and a smaller cyan "Change UTXO / Alice (1.5 BTC)" atom curving back toward Alice; behind everything a muted violet-to-teal nebula, low density, heavily defocused; dense starfield, dark near-field, cinematic, ultra detailed, legible small text labels; no robotic arm, no mechanical device, no syringe, no drill, no physical room or lab, no planet surface, no landscape, no people',
    motionPrompt:
  ...
  },
  {
    ...
    motionPrompt:
      "0–5s: transaction atoms light up as nodes on a violet web — a filament points from TX0 output[1] to TX1 input[0], which branches into output[0] and output[1]. 5–9s: the web branches outward into deep space like a cosmic filament structure. 9–13s: two filaments point at the same already-spent output; only one survives, the other collapses in red.",
  ...
  },
  {
    ...
    motionPrompt:
      "0–5s: macro warp-tunnel crash zoom into an output, streaks resolving on a glowing violet scriptPubKey lock condition. 5–11s: across the dark space, inside the next transaction's input, the Sig and PubKey particles of the scriptSig settle side by side as the proof, and one thin bright white-cyan filament snaps into place between that proof and the lock it satisfies; nothing travels along it — both ends respond together. 11–15s: opcodes materialize and orbit around the lock. Shallow depth of field.",
