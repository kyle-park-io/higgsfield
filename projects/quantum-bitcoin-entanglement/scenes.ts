...
export const scenes: Scene[] = [
  ...
  },
  {
    ...
    durationSeconds: 15, // narration ~13s
    keyframePrompt:
      "Astrophotography, photographed through a telescope on a long exposure: a mostly black deep-space field, with a faint dusty magenta-violet nebula drifting far behind everything, heavily out of focus and subtle — not vivid, not saturated; sparse stars over large areas of empty black. Several completed Bitcoin-gold block-molecules (warm orange-gold, hex #F7931A) hang scattered at clearly different heights and depths — one low and near, the next high and far back, the next lower again — a staggered zigzag strewn across the frame, each separated from its neighbours by a wide gap of empty black, the nearest one much larger than the furthest so the chain recedes with real depth. Each block-molecule is a rounded cluster of many small, individually visible glowing transaction orbs bonded together like a raspberry of separate spheres — the individual orbs are distinct and countable, never fused into one smooth mass — their surfaces turbulent and granulated like a real star's convecting photosphere; deep inside each cluster, clearly visible through the surrounding orbs, sits one bright faceted crystalline block-header core. From each cluster's header core, one short thin white-cyan filament reaches out at its own diagonal angle and touches the outer surface of the cluster before it — one separate segment per gap, so the filaments read as a broken zigzag strung between the clusters; nothing travels along the filament — both ends respond together; ₿ symbols on the clusters. Dark, restrained, subtle color, deep blacks, film grain, volumetric glow, cinematic, ultra detailed; no text labels, no planet surface, no landscape, no people; the ₿ Bitcoin symbol only — no Ethereum, no other cryptocurrency logos, no diamond or octahedron gem, no flat honeycomb hexagon lattice",
    motionPrompt:
  ...
  },
  {
    ...
    durationSeconds: 13, // narration ~11s
    keyframePrompt:
      'Scientific visualization, extreme macro: a tall vertical transparent stack-tube shaped from glowing violet energy (NOT lab glassware) floating in deep space — no laboratory, no benches, no monitors, no furniture; violet-magenta subatomic quark-like particles labeled "Sig" and "PubKey" stacked inside; the five script steps run up the tube as colliding particles, each step labeled with exactly one of these spellings and in this order from bottom to top — "OP_DUP", "OP_HASH160", "<PubKeyHash>", "OP_EQUALVERIFY", "OP_CHECKSIG" — these exact five strings and no other opcode names; a single green "TRUE" particle glowing at the top; behind everything a sparse deep teal and blue nebula so the violet stack reads clearly; dense starfield, dark near-field, volumetric glow, cinematic, ultra detailed, legible small text labels; no planet surface, no landscape, no people',
    motionPrompt:
