...
- Scene 3 nodes → floating holographic ring-scanner validators in the void (not server towers).
  Scene 4 now bridges the molecule→atom crash-zoom: faint gold parent block-molecule spheres frame
  the cyan transaction atom, so the 3→4 transition reads as a continuous zoom-through.
