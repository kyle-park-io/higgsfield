...
| 4→5 | atom→atom | same transaction atom continues | **cross-dissolve** (default) · end_chain optional | tested both. Dissolve keeps scene 4's locked hold; end_chain would need scene 4 re-generated to end pulling back to two atoms |
...
| 8→9 | quark→quark | stack → whole transaction atom (Super Dolly Out) | **end_chain** (or cross-dissolve) | scene 9 literally "pulls back from the stack" — a true continuous zoom-out. The one seam besides 4→5 where a morph is natural |
